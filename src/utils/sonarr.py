...
class Sonarr(object):
    ...
    def get_all_series(self):
        if int(round(self._all_series.get("ts", 0))) < int(round(time.time())) - 30:
            self.logger("Refreshing all series cache...")
    ...
    def get_all_tags(self):
        r = self._api_get("tag", {})
        self.logger(f"Result of API call to get all tags: {r}")
    ...
    def add_tag(self, tag):
        ...
        self.logger(f"Result of API call to add tag: {t}")
        return t
    ...
    def get_tag_id(self, tag):
        if i := next(
            ...
            self.logger(f"Found tag id [{i}] for tag [{tag}]")
            return i
        else:
            self.logger(f"No tag id found for [{tag}]; adding...")
            t = self.add_tag(tag)
    ...
    def lookup_quality_profile_id(self):
    ...
    def _api_get(self, endpoint, params={}):
        ...
        for k, v in params.items():
            url += f"&{k}={v}"
        self.logger(f"Submitting GET request: [{url}]")
        r = requests.get(url)
    ...
    def _api_post(self, endpoint, params={}):
        url = self.api_url.format(endpoint=endpoint)
        self.logger(f"Submitting POST request: [{url}]; params: [{params}]")
